...
void Calibrator::Calibrate()
{
  ...
  for (auto iter = pcs_.begin(); iter != pcs_.end(); iter++)
  {
    ...
    if (slave_id)
    {
      ...
      for (auto iter = cloud_after_Condition->begin(); iter != cloud_after_Condition->end(); iter++)
      {
        ...
        if (num > 0)
        {
          if (sqrt(pointNKNSquaredDistance[0]) < eps) // remove overlap
          {
      ...
      } // endror: have processed all inlier points
    }   // endif: salve id
    ...
    // have gotten z,roll,pitch ???
    curr_transform = init_ext * T_ms;
    registrator_->RegistrationByICP(curr_transform, transform); // calculate yaw
  ...
  } // endfor: have processed all lidars
}

            ...
            const Eigen::Matrix3d &img2ground_hmat)
  {
    ...
    for (int i = 0; i < 4; i++)
    {
      Eigen::Vector3d img_pt =
          ground2img_hmat_ * Eigen::Vector3d(ground_pts[i].x, ground_pts[i].y, 1);
      img_pt /= img_pt(2);
...
class Projector
{
 public:
                       ...
                       cv::Mat &current_frame, cv::Mat &bv_frame)
  {
    ...
    sort(points.begin( ), points.end( ), [](const Pt &a, const Pt &b) { return a.dist > b.dist; });
  ...
  void ProjectToRawImage(const cv::Mat &img, const Eigen::Matrix3d &K, const std::vector<double> &D,
                         const Eigen::Matrix4d &json_param, cv::Mat &current_frame, cv::Mat &bv_frame)
  {
    cv::Mat K1, D1, R1, T1;
                              ...
                              cv::Mat T)
  {
    ...
    cv::remap(img, outImg, mapX, mapY, cv::INTER_LINEAR);
    cv::Mat dist = oriCloud.rowRange(0, 1).mul(oriCloud.rowRange(0, 1)) + oriCloud.rowRange(1, 2).mul(oriCloud.rowRange(1, 2)) + oriCloud.rowRange(2, 3).mul(oriCloud.rowRange(2, 3));
    cv::Mat R_   = R;
    cv::Mat T_   = T;

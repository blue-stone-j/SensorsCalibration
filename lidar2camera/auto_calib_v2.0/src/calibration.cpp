...
Calibrator::Calibrator(const std::string mask_dir,
                       const std::string lidar_file, const std::string calib_file,
                       const std::string img_file, const std::string error_file)
{
  ...
  std::cout << "Initial error set to (r p y x y z):"
            << var[0] << " " << var[1] << " " << var[2] << " "
...
{
  // pre-filter points by initial extrinsic and store points that can be projected onto image
  pcl::PointCloud<pcl::PointXYZI>::Ptr pc_filtered(new pcl::PointCloud<pcl::PointXYZI>);
  int margin = 300;
  float intensity_max = 1;
  // float intensity_mean = 0;
  std::vector<Var6> vars;            // store all possible values
  Util::GenVars(1, 5, 1, 0.5, vars); // compute all possible values
                            ...
                            std::vector<pcl::PointIndices> &seg_indices)
{
  ...
  while (plane_size > 2000)
  {
    ...
    indices_plane_all->indices.insert(indices_plane_all->indices.end(),
                                      indices_plane->indices.begin(), indices_plane->indices.end());
...
{
  VisualProjection(init_extrinsic_, img_file_, "init_proj.png"); // get undistort image
...
void Calibrator::BruteForceSearch(int rpy_range, float rpy_resolution,
                                  int xyz_range, float xyz_resolution, bool is_coarse)
{
...
void Calibrator::PrintCurrentError()
{
  ...
  std::cout << "Error:"
            << RAD2DEG(Util::GetRoll(error_T)) << " " << RAD2DEG(Util::GetPitch(error_T)) << " " << RAD2DEG(Util::GetYaw(error_T))
            << " " << Util::GetX(error_T) << " " << Util::GetY(error_T) << " " << Util::GetZ(error_T)
            << std::endl;
}
...
void Calibrator::VisualProjectionSegment(Eigen::Matrix4f T, std::string img_file, std::string save_name)
{
  ...
  // project points to image and reserve seg id
  for (const auto &src_pt : pc_->points)
  {
    Eigen::Vector4f vec;
    vec << src_pt.x, src_pt.y, src_pt.z, 1;

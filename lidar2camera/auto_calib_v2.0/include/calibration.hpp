...
{
public:
  Calibrator(const std::string mask_dir,
             const std::string lidar_file, const std::string calib_file, const std::string img_file, const std::string error_file);
  void Calibrate();
...
private:
  ...
  std::vector<double> dist_; // to undistort
  float max_score_ = 0;

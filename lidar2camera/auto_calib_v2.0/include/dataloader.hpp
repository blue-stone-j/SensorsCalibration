...
public:
  ...
  static void LoadMaskFile(const std::string mask_dir, const Eigen::MatrixXf intrinsic,
                           const std::vector<double> dist, cv::Mat &masks,
  ...
  static void LoadCalibFile(const std::string filename, Eigen::MatrixXf &intrinsic,
                            Eigen::Matrix4f &extrinsic, std::vector<double> &dist)
  {
    ...
    if (elements.size() == 9)
    { // data is row-major and map is column-major
    ...
    getline(file, line);
    ss = std::stringstream(line);
    ss >> tmpStr >> extrinsic(0, 0) >> extrinsic(0, 1) >> extrinsic(0, 2) >> extrinsic(0, 3) >> extrinsic(1, 0) >> extrinsic(1, 1) >> extrinsic(1, 2) >> extrinsic(1, 3) >> extrinsic(2, 0) >> extrinsic(2, 1) >> extrinsic(2, 2) >> extrinsic(2, 3);
  }

    ...
    const pcl::PointCloud<pcl::PointXYZI>::Ptr register_cloud)
{
  ...
  Eigen::Vector3d ea = rotation_matrix.eulerAngles(2, 1, 0); // order of axis
  float target_extrinsic_vec[6] = {0}, curr_optim_extrinsic_vec[6] = {0};
  bool is_violence_search = false;
  if (is_violence_search)
  {
    float rpy_resolution = 1; // degree
    float xyz_resolution = 0.1;
    // correction component should be limited in a small range
...
float Optimizer::MaskRegistrationLoss(const float var[6], const cv::Mat *distance_img,
                                      const pcl::PointCloud<pcl::PointXYZI>::Ptr register_cloud)
{
  ...
  for (const auto &src_pt : register_cloud->points)
  {
    ...
    Eigen::Vector4d cam_point = T * vec; // point in camera frame
    Eigen::Vector3d cam_vec;
    cam_vec << cam_point(0), cam_point(1), cam_point(2);
    Eigen::Vector3d vec_2d = curr_optim_intrinsic_ * cam_vec; // project point onto image
    if (vec_2d(2) > 0)
    {
      ...
      if (x >= 0 && x < distance_img->cols && y >= 0 && y < distance_img->rows)
      {
        ...
        double pixel_gray = (f3 + f2 + f1) / 3; // cooresponding pixel
  ...
  } // endfor: have processed whole cloud
  // cv::imwrite("before_refine.png",image);
  return (float)PointCnt / register_cloud->points.size();
}

// match points and image and compute ratio of matched point w.r.t all points
float Optimizer::CostFunction(const float var[6], const cv::Mat *distance_img,
                              const pcl::PointCloud<pcl::PointXYZI>::Ptr register_cloud)
{
...
void Optimizer::SaveProjectResult(const std::string img_name, const cv::Mat *distance_img,
                                  const pcl::PointCloud<pcl::PointXYZI>::Ptr register_cloud)
{
...
void Optimizer::random_search_params(int search_count, float delta_x, float delta_y, float delta_z,

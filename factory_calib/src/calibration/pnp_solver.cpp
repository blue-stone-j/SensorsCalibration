...
struct XCostFunctorPnP
{
  ...
  {
    return new ceres::AutoDiffCostFunction<XCostFunctorPnP, 2, 3, 3>(new XCostFunctorPnP(objpoint, imgpoint, camera_intrinsic, dist_coeffs, tra));
  }
};
...
struct ZCostFunctorPnP
{
  ...
  static ceres::CostFunction *create(std::vector<float> objpoint,
                                     std::vector<float> imgpoint, std::vector<std::vector<double>> camera_intrinsic, std::vector<double> dist_coeffs, double *tra)
  {
    return new ceres::AutoDiffCostFunction<ZCostFunctorPnP, 2, 3, 3>(new ZCostFunctorPnP(objpoint, imgpoint, camera_intrinsic, dist_coeffs, tra));
  }
};
...
bool solveCamPnP(std::vector<std::vector<float>> &objpoints, std::vector<std::vector<float>> &imgpoints,
                 std::vector<std::vector<double>> &camera_intrinsic, std::vector<double> &dist_coeffs,
    ...
    std::vector<Eigen::Vector3d> *camera_control_points)
{
  ...
  Eigen::Matrix<double, 12, 1> vec =
      betas[0] * eigen_vectors.block(0, 0, 12, 1) + betas[1] * eigen_vectors.block(0, 1, 12, 1) + betas[2] * eigen_vectors.block(0, 2, 12, 1) + betas[3] * eigen_vectors.block(0, 3, 12, 1);
    ...
    std::vector<Eigen::Vector3d> *pts3d_camera)
{
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    Eigen::Vector3d ptc = camera_control_points[0] * alphas[0] + camera_control_points[1] * alphas[1] + camera_control_points[2] * alphas[2] + camera_control_points[3] * alphas[3];

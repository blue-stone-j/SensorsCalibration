...
bool CalibrationBoardTool::ChessBoardCalibration(const std::vector<std::vector<float>> &gray_img,
                                                 ...
                                                 CalibrationOutputParams *calibration_result)
{
  ...
  bool is_success = corner_detect.CornerDetection(gray_img, &board_center_point_x, &board_center_point_y,
                                                  &slope_point1, &slope_point2,
                                                  &grid_x_dis, &grid_y_dis, &grid_center_points);
  if (!is_success)
  {
  ...
  bool homography_flag              = CeresSolveHomography(img_quad, world_quad,
                                                           expected_symmetric_distance, &homo_mat);
  if (!homography_flag)
  {
  ...
  if (std::fabs(pA_mid_img.y - vanishing_point(1, 0)) < 1e-6 || std::fabs(pB_mid_img.y - vanishing_point(1, 0)) < 1e-6)
  {
    LOGE("wrong param");
    return false;
  }
  ...
  homography_flag = CeresSolveHomography(img_quad_ground, world_quad_ground,
...
bool CalibrationBoardTool::VerticalBoardCalibration(const std::vector<std::vector<float>> &gray_img,
                                                    const InputParam &calibration_inputs,
                                                    const std::string &output_json_path,
                                                    CalibrationOutputParams *calibration_result)
{
  ...
  homography_flag = CeresSolveHomography(ground_img_pts, ground_world_pts,
    ...
    CalibrationOutputParams *calibration_result)
{
  ...
  // compute image to board homography
  float expected_symmetric_distance = 0.02;
  bool homography_flag              = CeresSolveHomography(img_quad, board_quad,
                                                           expected_symmetric_distance, &img2board_hmat);
  if (!homography_flag)
  {
    LOGE("failed to compute homography");
    return false;
...
bool CalibrationBoardTool::SaveCalibrationResult(const CalibrationOutputParams &calibration_result,
                                                 const std::string &output_json_path)
{
...
bool CalibrationBoardTool::SaveJsonCalibration(const CalibrationOutputParams &result,
    ...
    CameraIntrinsic *camera_intrinsic)
{
  ...
  {
    camera_intrinsic->camera_focus_length_fx = read_data["front_center_camera_calib"]["param"]["cam_K"]["data"][0][0]
                                                   .asDouble( );
    camera_intrinsic->camera_focus_length_fy = read_data["front_center_camera_calib"]["param"]["cam_K"]["data"][1][1]
                                                   .asDouble( );

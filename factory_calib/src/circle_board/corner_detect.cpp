...
void CornerDetector::EstimateGrayThres(const std::vector<std::vector<float>> &img_gray,
                                       std::vector<int> *thresh_box)
{
  thresh_box->clear( );
  ...
  int stride = static_cast<int>((127.5 - abs(127.5 - aver_gray)) * (1.0 - (127.5 - aver_gray) * (127.5 - mid_gray) / static_cast<float>(127.5 * 127.5)) / 6.0);
...
bool CornerDetector::clusterLines(const std::vector<Circle> &circles,
                                  ...
                                  CornerPoints *corner_pts)
{
  ...
  for (auto black_pair : possible_black_lines)
  {
    ...
    for (int i = 0; i < line_num; ++i)
    {
      ...
      if (line_pos_idx < 0 || line_pos_idx >= board_pattern_.width || line_pos_idx == mid_index || line_pos_idx == mid_index - 1)
      {
        continue;
      }
...
bool CornerDetector::findTwoBlackLines(const std::vector<Circle> &circles,
                                       const std::vector<std::shared_ptr<LineModel>> &line_models,
                                       std::vector<BlackLines> *possible_black_lines)
{
...
bool CornerDetector::checkLinePattern(const std::vector<Circle> &circles, const int &left_black_circle_idx,
                                      ...
                                      CornerPoints *corner_pts)
{
  ...
  for (int i = 0; i < static_cast<int>(line_pos_index.size( )); ++i)
  {
    ...
    for (int p = 0; p < pt_num; ++p)
    {
      ...
      // skip black circles
      if ((i == mid_index - 1 && circle_idx == left_black_circle_idx) || (i == mid_index && circle_idx == right_black_circle_idx))
      {
        valid_pt_num++;
        continue;
      }

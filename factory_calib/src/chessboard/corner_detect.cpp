...
void CornerDetect::ImageThreshold(const std::vector<std::vector<float>> &imgGray,
...
void CornerDetect::EstimateGrayThres(const std::vector<std::vector<float>> &imgGray,
                                     std::vector<int> *thres_box, float edge_fill_percent)
{
  ...
  std::vector<int> gray_pixel_num(256);      // num of pixels that has some gray
  std::vector<float> gray_prob(256);         // ratio of correspending gray
  std::vector<float> gray_distribution(256); // num of pixels whose gray less than index
  int pixel_num = 0;
  ...
  int stride = static_cast<int>((127.5 - abs(127.5 - aver_gray))
                                * (1.0 - (127.5 - aver_gray) * (127.5 - mid_gray) / static_cast<float>(127.5 * 127.5))
                                / 6.0); //???
...
// Binarize image???
void CornerDetect::ImageDilate(const std::vector<std::vector<int>> thresh_vec, int kernal[3][3],
                               std::vector<std::vector<int>> *dilation_vec, float edge_fill_percent)
{
...
void CornerDetect::FindContours(const std::vector<std::vector<int>> dilation_vec,
                                std::vector<std::vector<Point>> *rt_contours, float edge_fill_percent)
{
  ...
  for (size_t i = 1; i < img.size( ) - 1; i++)
  {
    ...
    {
      if (img[i + 1][j] == 0 && img[i + 1][j - 1] == 0 && img[i + 1][j + 1] == 0
...
bool CornerDetect::CornerDetection(const std::vector<std::vector<float>> &imgGray, int *vp_x, int *vp_y,
                                   ...
                                   std::vector<Point> *grid_center_points)
{
  ...
  float edge_fill_percent = 0.05; // devide gray into 20 ranges???

...
bool CornerDetector::detect(const std::vector<std::vector<float>> &gray_img,
                            const VerticalBoard &lp, CornerPoints *detected_corner_points)
{
...
void CornerDetector::displayDetectedLines(const cv::Mat &img,
...
bool CornerDetector::selectLinesByLineRelation(const std::vector<std::shared_ptr<LineModel>> &lines,
                                               ...
                                               std::vector<std::vector<int>> *pattern_idxs)
{
  ...
  for (size_t i1 = 0; i1 < lines.size( ); ++i1)
  {
    ...
    for (size_t lridx = 0; lridx < left_right_idxs.size( ); ++lridx)
    {
      ...
      if (this->checkLineCluster(c_line, lines[l_line_idx], lines[r_line_idx], estimated_pt_dist,

...
void LidarDetector::LidarDetection(std::string pcds_dir)
{
  ...
  // Plane segmentation to get points on board
  ...
  float a_final = coefficients->values[0] / coefficients->values[3]; // plane coefficients
  ...
  for (pcl::PointCloud<pcl::PointXYZ>::iterator pt = edges_cloud->points.begin( );
  ...
  // take it as binary image
  std::vector<std::vector<bool>> pro_map(int(max_pt_y * 200) - int(min_pt_y * 200) + 1,
  ...
  // find a point that has most neighbors
  ...
  // traverse all coarse centers to refine center
  for (pcl::PointCloud<pcl::PointXYZ>::iterator pt = initial_centers->points.begin( );
       pt < initial_centers->points.end( ); ++pt)
  {
    ...
    int final_i, final_j;
    // center has least neighbors than center's neighbors
    // traverse center's neighbors in rectangle
    for (int i = initial_y - 20; i <= initial_y + 20; i++)
    {
      for (int j = initial_x - 20; j <= initial_x + 20; j++)
      {
        int cnt = 0;
        // find neighbors in round
    ...
    } // endfor: have computed refined center
  ...
  } // endfor: have computed all refined centers

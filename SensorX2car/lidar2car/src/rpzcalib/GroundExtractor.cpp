  ...
  for (; iter_cnt < lpr_max_iters_; iter_cnt++)
  {
    ...
    for (size_t j = 0; j < in_cloud->points.size( ); j++)
    {
      ...
      double point_to_plane_dis = std::fabs(
          plane->normal(0) * point.x + plane->normal(1) * point.y + plane->normal(2) * point.z + plane->intercept);
...
bool GroundExtractor::RandomRansacFitting(const PointCloudPtr in_cloud,
                                          PointCloudPtr g_cloud, PointCloudPtr ng_cloud,
                                          PlaneParam *plane)
{
  ...
  for (int i = 0; i < rr_iter_times_; i++)
  {
    for (int j = 0; j < rr_max_rand_iters_; j++)
    {
      ...
      {
        PointType point           = in_cloud->points[j];
        double point_to_plane_dis = std::fabs(pp.normal(0) * point.x
                                              + pp.normal(1) * point.y
                                              + pp.normal(2) * point.z
                                              + pp.intercept);
  ...
  {
    PointType point           = in_cloud->points[j];
    double point_to_plane_dis = std::fabs(best_plane.normal(0) * point.x
                                          + best_plane.normal(1) * point.y
                                          + best_plane.normal(2) * point.z
                                          + best_plane.intercept);
    if (point_to_plane_dis <= rr_fit_dist_thre_)
    {
      g_cloud->points.emplace_back(point);
    }
...
bool GroundExtractor::RandomSearchPlane(const PointCloudPtr in_cloud,
                                        PlaneParam &best_plane, int &max_inlier_points,
                                        ...
                                        int iteration_times)
{
  ...
  for (int i = 0; i < iteration_times; i++)
  {
    ...
    {
      PointType point           = in_cloud->points[j];
      double point_to_plane_dis = std::fabs(tmp_plane.normal(0) * point.x
                                            + tmp_plane.normal(1) * point.y
                                            + tmp_plane.normal(2) * point.z
                                            + tmp_plane.intercept);
      if (point_to_plane_dis <= rr_fit_dist_thre_)
      {
        inlier_points++;

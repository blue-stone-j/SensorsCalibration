                                         ...
                                         float thres)
{
  ...
  {
    if (cloud_in->points[i].x * cloud_in->points[i].x + cloud_in->points[i].y * cloud_in->points[i].y + cloud_in->points[i].z * cloud_in->points[i].z < thres_square)
    {
      continue;
    }
...
void Registrator::inputFrame(PointCloud::Ptr cloud, //  const PointCloudFeatures& input_features,
      ...
      for (int i = 0; i < cornerPointsSharpNum; ++i)
      {
        ...
        if (pointSearchSqDis[0] < distance_sq_threshold_)
        {
          ...
          for (int j = closestPointInd + 1; j < (int)laserCloudCornerLast_->points.size( ); ++j)
          {
            ...
            double pointSqDis = (laserCloudCornerLast_->points[j].x - pointSel.x) * (laserCloudCornerLast_->points[j].x - pointSel.x) + (laserCloudCornerLast_->points[j].y - pointSel.y) * (laserCloudCornerLast_->points[j].y - pointSel.y) + (laserCloudCornerLast_->points[j].z - pointSel.z) * (laserCloudCornerLast_->points[j].z - pointSel.z);
        ...
        if (minPointInd2 >= 0) // both closestPointInd and minPointInd2 is valid
        {
          ...
          double s;
          if (distortion_)
            s = (input_features.corner_sharp_pts->points[i].intensity - int(input_features.corner_sharp_pts->points[i].intensity))
                / scan_period_;
          else
            s = 1.0;
      ...
      for (int i = 0; i < surfPointsFlatNum; ++i)
      {
        ...
        if (pointSearchSqDis[0] < distance_sq_threshold_)
        {
          ...
          for (int j = closestPointInd + 1; j < (int)laserCloudSurfLast_->points.size( ); ++j)
          {
            ...
            double pointSqDis = (laserCloudSurfLast_->points[j].x - pointSel.x) * (laserCloudSurfLast_->points[j].x - pointSel.x) + (laserCloudSurfLast_->points[j].y - pointSel.y) * (laserCloudSurfLast_->points[j].y - pointSel.y) + (laserCloudSurfLast_->points[j].z - pointSel.z) * (laserCloudSurfLast_->points[j].z - pointSel.z);
          ...
          if (minPointInd2 >= 0 && minPointInd3 >= 0)
          {
            ...
            double s;
            if (distortion_)
              s = (input_features.surf_flat_pts->points[i].intensity - int(input_features.surf_flat_pts->points[i].intensity)) / scan_period_;
            else
              s = 1.0;

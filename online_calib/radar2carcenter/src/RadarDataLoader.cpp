...
void RadarDataLoader::getRadarCalibData(const std::string &radar_file_dir, const std::string &radar_type,
                                        const std::string &novatel_inspva, std::vector<RadarFrame> &radar_frames,
                                        int start_radar_file_num, int max_radar_file_num)
{
  ...
  {
    if (!getDelphiRadarFrame(radar_file_dir, radar_files, inspva_times, inspva_speed, radar_frames)) { exit(1); }
  }
  else if (radar_type == "conti")
  {
    if (!getContiRadarFrame(radar_file_dir, radar_files, inspva_times, inspva_speed, radar_frames)) { exit(1); }
  }
  else
  {

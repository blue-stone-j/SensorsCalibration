...
void RadarCalibrator::calib(const std::string &radar_file_dir, const std::string &radar_type,
                            const std::string &novatel_inspva, const std::string &output_dir,
                            const RadarCalibParam &param, bool guessRoughYaw, bool savefig)
{
  // set parameters
  ...
  data_loader.getRadarCalibData(radar_file_dir, radar_type, novatel_inspva, radar_datas,
                                param.start_file_num, param.max_file_num);
  ...
  else
  {
    ...
    for (int i = 0; i < data_num; ++i)
    {
      ...
      for (size_t turn = 0; turn < iter; ++turn)
      {
        // get stationary based on max velocity and provided vehicle speed
        double new_yaw;
        if (!getStationary(start_yaw, data.ve, data.vi, data.track_angle, stationary_idxs)) { break; }
        // if (used_frm_ % 10 == 0 && used_frm_ > estimate_iter) {
        //     saveStationaryTxt(data.track_angle, data.track_dist, data.times,
        //                     stationary_idxs, 32*M_PI/180);
        // }
        if (!fittingCosine(start_yaw, data.ve, data.vi, data.track_angle, stationary_idxs, new_yaw, applyWeight)) { break; }
        // if (used_frm_ < estimate_iter)
        start_yaw = new_yaw;
        // narrow range to pick out stationary object
        if (used_frm_ == estimate_iter / 2.0)
                                  ...
                                  double &rough_yaw)
{
  ...
  std::sort(sort_idx.begin( ), sort_idx.end( ), [&probability](size_t i1, size_t i2) { return probability[i1] > probability[i2]; });
...
bool RadarCalibrator::getStationary(const double &yaw, const std::vector<double> &ve, const std::vector<double> &vi,
                                    const std::vector<double> &track_angle, std::vector<int> &stationary_indexs)
{
...
bool RadarCalibrator::fittingCosine(const double &yaw, const std::vector<double> &ve, const std::vector<double> &vi,
                                    const std::vector<double> &track_angle, const std::vector<int> &selected_indexs,
                                    double &new_yaw, bool applyWeight)
{
                                        ...
                                        const double &yaw)
{
  ...
  // }
  std::string output_file = "./stationary/" + std::to_string(used_frm_) + "_" + std::to_string(selected_indexs.size( )) + ".txt";
...
void RadarCalibrator::saveFitlineTxt(const std::string &output_file,
                                     const std::vector<double> &ve, const std::vector<double> &vi,
                                     const std::vector<double> &track_angle, const std::vector<int> &selected_indexs,
                                     const double &A, const double &yaw_deg)
{
  std::ofstream out_file(output_file);
  if (!out_file.is_open( ))
  {

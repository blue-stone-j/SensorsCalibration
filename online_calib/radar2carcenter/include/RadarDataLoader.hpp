    ...
    const std::string &time_string, double &time_count_s)
{
  ...
  std::string new_string = elements[0] + "-" + elements[1] + "-" + elements[2] + " " + elements[3] + ":" + elements[4] + ":" + elements[5] + "." + elements[6];
...
class RadarDataLoader
{
 public:
  ...
  void getRadarCalibData(const std::string &radar_file_dir, const std::string &radar_type,
                         const std::string &novatel_inspva, std::vector<RadarFrame> &radar_frames,
                         int start_radar_file_num = 100, int max_radar_file_num = 1000);
